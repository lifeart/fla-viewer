...
if (!elementName || !drawingName) {
  console.error('Usage: node scripts/rank-tvg-shapes-by-diff.mjs <elementName> <drawingName> [layerType] [limit]');
  process.exit(1);
}
    ...
    for (let index = 0; index < layer.shapes.length; index++) {
      ...
      for (let y = 0; y < 160; y++) {
        for (let x = 0; x < 160; x++) {
          ...
          if (candFg && !refFg) {
            extraOverlap++;
          } else if (candFg && refFg) {
            const dr = Math.abs(refData[idx + 0] - shiftedData[idx + 0])
              + Math.abs(refData[idx + 1] - shiftedData[idx + 1])
              + Math.abs(refData[idx + 2] - shiftedData[idx + 2]);
            if (dr <= 50) matchedOverlap++;
            else mismatchOverlap++;
          }
        }
      }
      if (area === 0) continue;
      rows.push({
        ...
        mismatchOverlap,
        matchedOverlap,
        badOverlap: extraOverlap + mismatchOverlap,
        badRatio: (extraOverlap + mismatchOverlap) / area,
        componentCount: layer.shapes[index].components.length,
      });
    }

    rows.sort((a, b) => b.badOverlap - a.badOverlap || b.badRatio - a.badRatio);
