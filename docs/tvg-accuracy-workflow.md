...
- Rejected probe: lowering the mid-alpha edge-pixel gate from `500` to `400` includes `4bf5/4bf5-1` in the edge-tone pass and improves the Puppeteer benchmark path (`97.8906 -> 98.3789`), but the direct Vitest sample guard rendered worse (`97.7344`). Keep the `500` gate until this script/test discrepancy is explained by a deterministic source or browser-rendering difference.
