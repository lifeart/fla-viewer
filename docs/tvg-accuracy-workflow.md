...
- Accepted correction to that caveat: the earlier renderer probe classified edge pixels from the alpha mask after dense coverage expansion, while the positive output diagnostic used the original transparent alpha. Preserving the pre-coverage mask and applying final-RGB edge tone reproduces the diagnostic safely when gated by the existing structural classifier for exactly one priority-2 full cross-paint carrier. A subtract sweep found `22` was the raw optimum for `color-21` (`+0.1484` versus disabled), while the seven dense-cluster guards were unchanged.
- Browser-stability evidence: alpha-count eligibility is rejected for this pass because `color-21` has `921` fractional pixels in Puppeteer Chrome but `1142` in the Vitest Chromium. The priority-2 source classifier activates the same correction in both; the Vitest raw gain versus disabled is smaller but positive (`+0.0391`).
- Full benchmark after priority-2 pre-coverage edge tone: trusted source-fresh vector floor improved `97.41 -> 97.56` at `color.101/color-21`; trusted/source-fresh/raw rounded averages, benchmark classes, and bitmap scores remained unchanged. Use `npm run benchmark:tvg:raw -- --dense-priority2-carrier-edge-tone-subtract=0` for the explicit disabled comparison.
